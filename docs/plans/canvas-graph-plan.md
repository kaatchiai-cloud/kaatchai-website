...
**File:** `mocks/canvas-preview.html` (new, root-level, throwaway)
...
**IMPORTANT:** the Phase 1 `mocks/canvas-preview.html` mock is a layout-validation prototype, NOT the visual target. Final UI must match Stori's Aurora theme polish level. Mock-grade visuals are unacceptable for ship.
...
| File | Phase | Change |
|---|---|---|
| `mocks/canvas-preview.html` | 1 | NEW — static mockup |
...
- [ ] Agent assigned to Phase 1 builds `mocks/canvas-preview.html` and pauses for visual approval
