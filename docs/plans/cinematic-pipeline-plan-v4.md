...
Status: implemented (Phase 12 cleanup pending). Supersedes v3, v2, v1. **Updated in place after two audit passes: v4 pre-fix (20 issues) and v4 post-audit (3C+4H+3M+5O).** See §0.1 and §0.2 for audit-fix changelogs.
...
- Scene-level `speakerVisible` field — deprecated in Phase 3.5, removed in Phase 12. Also remove the `scene.speakerVisible === false` fallback at [js/29-canvas-render.js:596](js/29-canvas-render.js#L596).
- `durationStatus` and `durationDriftPct` field writes at [js/33-audio-rehearsal.js:533-534](js/33-audio-rehearsal.js#L533) and the `durationStatus` read at [line 738](js/33-audio-rehearsal.js#L738) — remove together with `computeDurationStatus`.
- `STYLE_PRESETS` reading paths at [js/17c-create-pipeline.js:11-12](js/17c-create-pipeline.js#L11) — replace with `createJobState.subStyle` reads (§17.3). The global itself stays.
- `legacyDialogueToLine` migration function ([js/17b-create-references.js](js/17b-create-references.js)) — remove once all known projects have been restored through the migration path and `scene.dialogue` → `scene.dialogueLines` conversion is confirmed complete across the user base.
...
### 17.4 Architectural note — shim → one-time migration

The plan originally specified `Object.defineProperty` getter shims to provide backward-compat reads of `scene.dialogue` and `scene.duration` while new code migrated to `scene.dialogueLines` and `scene.durationSec`. During implementation this approach was replaced with a one-time migration in the `15-project.js` restore path: on load, old fields are converted to the canonical fields and deleted.

**Why this is cleaner:** No silent runtime traps. No getter installed on every scene object. No ambiguity about which read sites have migrated. The restore path runs once per project load and leaves only canonical fields.

**Consequence for telemetry:** The `_cinematicShimWarnings` counters (`dialogue`, `durationFallthrough`, `timingFallback`) can never be incremented — there is no shim to trigger them. They have been removed. **Open decision:** add explicit migration-error logging to the restore path (`15-project.js:1090–1106`) to capture cases where a project save has unexpected field combinations (e.g., both `dialogue` and `dialogueLines` present, or `duration` present without `durationSec`). This would replace the telemetry the shim approach would have provided. No action taken yet — log if the need arises in production.
