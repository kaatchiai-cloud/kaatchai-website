...
> **Source:** `/Users/praveen/Desktop/stori/migration-plan.md` Part 2 row 06; coverage matrix rows 9, 10; [OVERRIDES] O7, O13, O15.
...
### In scope
1. **Fresh grep before any deletion** — re-verify the actual reference counts across **all six provider key surfaces** (audit 2026-05-06 caught that the original count missed five of them). The fresh grep at phase doc authoring time returned:

   | Provider key | localStorage key(s) | Direct provider URL | Verified call sites |
   |---|---|---|---|
   | Gemini | `stori_key_paid`, `stori_key_free` | `generativelanguage.googleapis.com` | `js/15-project.js`, `js/24-photopilot.js`, `js/17a-create-api.js`, `js/20-reels-creator.js`, `js/28-canvas-consistency.js`, `js/21-kling.js`, `js/17c-create-pipeline.js`, `js/17d-create-languages.js` |
   | Kling | `stori_kling_access_key`, `stori_kling_secret_key` | (proxied via `api/kling.js`) | `js/21-kling.js:13–15`, `js/17a-create-api.js:822–823` |
   | ElevenLabs | `stori_elevenlabs_key` | `api.elevenlabs.io` | `js/17a-create-api.js:296, 583, 636, 810` |
   | OpenAI | `stori_openai_key` | `api.openai.com` direct fetch | `js/26b-llm-router.js:46, 60–64` |
   | Anthropic | `stori_anthropic_key` | `api.anthropic.com` direct fetch | `js/26b-llm-router.js:78, 94–103` |
   | fal.ai (Tier-2 lipsync) | (BYOK in keys panel; check exact key name at kickoff) | `fal.run/fal-ai/...` | `js/17c-create-pipeline.js:3721–3900` (Tier-2 lipsync via fal.ai) |

   The earlier "36 refs / 19 fetch sites / 4 files" figures from `migration-details.md` were doubly stale — they cited only the Gemini surface and predate the 14 newer JS files. **Re-run all six greps at phase kickoff** and itemize remaining references. By the time this phase starts, Phases 05 + 06 should have driven Gemini, Kling, ElevenLabs, fal.ai counts to ~0 in pipeline files; OpenAI + Anthropic in `26b-llm-router.js` are P06 Brainstorm extraction territory.

2. **BYOK key deletion** — broaden from "two legacy Gemini key names" to **all browser-stored provider secrets**:
   - Remove every `localStorage.getItem|setItem` of: `stori_key_paid`, `stori_key_free`, `stori_kling_access_key`, `stori_kling_secret_key`, `stori_elevenlabs_key`, `stori_openai_key`, `stori_anthropic_key`, and the fal.ai key (verify exact name at kickoff).
   - **(rev-4) Delete the full key-getter + callGeminiAPI + trackCost suite from `js/17a-create-api.js`:**
     ...
     - **`trackCost()`** — multiple call sites (`js/17a-create-api.js:472, 730, 785`); sweep all three call sites + the function definition. The dollar-cost UI removal at item 3 makes `trackCost` an orphan; delete it.
   - Remove the BYOK input fields from `index.html` for all six providers (the "API key" `<input>` elements + their containing settings panels — there are multiple panels per `index.html:2184–2193, 2954–2963` and elsewhere).
...
4. **Direct AI fetch deletion sweep** — every remaining direct `fetch('https://generativelanguage.googleapis.com/...')` and direct Kling URL replaced or removed. Anything that's a now-dead code path (e.g., a fallback that triggered when no API key was set) is deleted entirely, not commented out.
...
|---|----------------|--------------|
| 1 | **Zero browser-stored provider secrets.** `grep -rnE "stori_key_paid\|stori_key_free\|stori_kling_(access\|secret)_key\|stori_elevenlabs_key\|stori_openai_key\|stori_anthropic_key\|stori_fal" js/ index.html` returns 0 hits. | grep |
| 2 | **Zero direct provider fetches.** `grep -rnE "generativelanguage\\.googleapis\\.com\|api\\.openai\\.com\|api\\.anthropic\\.com\|api\\.elevenlabs\\.io\|fal\\.run" js/ index.html` returns 0 hits. | grep |
| 3 | `grep -rnE "getCreateGeminiKey\|getPPApiKey\|getReelApiKey\|getFreeKey\|getPaidKey\|getReelFreeKey\|getReelPaidKey\|callGeminiAPI\|trackCost" js/ index.html` returns 0 hits. | grep |
| 4 | BYOK input fields for all six providers (Gemini, Kling, ElevenLabs, OpenAI, Anthropic, fal.ai) removed from `index.html`. | DOM inspection + grep on `<input>` ids. |
...
- [ ] At phase kickoff (NOT at this doc's authoring time), run:
  ```bash
  grep -rn "stori_key_paid\|stori_key_free" js/ index.html
  grep -rn "generativelanguage.googleapis.com" js/ index.html
  grep -rn "getCreateGeminiKey" js/ index.html
  grep -rn "createDollarCost\|estimateCost\|costInDollars" js/ index.html  # dollar-cost UI helpers
  ```
...
- [ ] For each file in the grep output:
  - Delete the `localStorage.getItem|setItem('stori_key_paid'|'stori_key_free'|'stori_kling_access_key'|'stori_kling_secret_key'|'stori_elevenlabs_key'|'stori_openai_key'|'stori_anthropic_key'|'stori_fal*', ...)` call sites.
...
- [ ] **(rev-4) `js/17a-create-api.js` — delete the full key-getter + callGeminiAPI + trackCost suite:**
  ...
  - `trackCost()` — sweep the 3 call sites at `:472, :730, :785` AND the function definition. (Cost UI removal at §5.3 makes the column orphan.)
- [ ] **(rev-4) `js/21-kling.js` — confirm P05 already deleted `generateKlingJWT()` + the two `stori_kling_*` reads at `:13–14`.** If anything remains, sweep here.
...
- [ ] Delete them.
- [ ] For any `id="cost-estimate"` / `id="cost-display"` element, remove from `index.html`.
...
- [ ] For each remaining `fetch('https://generativelanguage.googleapis.com/...')` after Phases 05+05, decide: (a) replace with `callApi(...)` if the call is still needed, (b) delete if it was a fallback for the now-removed BYOK case. Default: delete.
- [ ] For direct Kling URL calls in `js/21-kling.js` (likely already replaced by P05) — re-confirm zero remaining.
- [ ] Re-run the four grep targets from §5.1 — must all be 0.
...
### Predecessors
- **Phase 06** must exit so all six secondary pipelines route through `/v1/*` before deletion.
...
- Modified: `js/15-project.js`, `js/24-photopilot.js`, `js/17a-create-api.js`, `js/20-reels-creator.js`, `js/28-canvas-consistency.js`, `js/21-kling.js`, `js/17c-create-pipeline.js`, `js/17d-create-languages.js`, `index.html`.
...
- Forbidden: `migration-details.md`, `app/redesign-plan.md`, `app/*-mobile-mockup*.html` — historical / mobile inputs.
...
- Predecessor: `/Users/praveen/Desktop/stori/migration-phase-06-secondary-pipelines-extraction.md`
- Successor: `/Users/praveen/Desktop/stori/migration-phase-08-production-launch.md`
- Source spec: `/Users/praveen/Desktop/stori/migration-details.md` §Modified Files L177–232 (BYOK + cost rows), §Deleted Code L236–244 (line counts re-verified — see §1 fresh grep). All Stripe/quota rows excluded per O15.
