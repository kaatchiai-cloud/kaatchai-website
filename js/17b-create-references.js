...
// Returns true when the selected template is photorealistic or cinematic.
// In these modes, uploading real person photos is blocked — characters and
// avatars must use AI-generated appearances only to prevent deepfakes.
function _isPhotorealisticMode() {
  if (typeof TEMPLATES === 'undefined' || typeof selectedTemplate === 'undefined' || !selectedTemplate) return false;
  const tpl = TEMPLATES.find(t => t.id === selectedTemplate);
  return !!(tpl && (tpl.style === 'photorealistic' || tpl.style === 'cinematic'));
}
  ...
  charCardsEl.innerHTML = storyCharacters.map(ch => `
    <div class="ref-card" data-char-id="${ch.id}">
      ...
      ${ch.imgDataUrl
        ...
        : `<div class="ref-card-placeholder"${_isPhotorealisticMode() ? '' : ` data-upload-char="${ch.id}"`}>${_isPhotorealisticMode() ? 'AI-generated portrait' : 'Click to upload illustration'}</div>`}
      ...
      ${_isPhotorealisticMode() ? `<div style="font-size:0.65rem;color:var(--text-muted);padding:2px 0;text-align:center;">AI-generated appearance only</div>` : (!ch.imgDataUrl ? `<button class="char-upload-btn" data-upload-char="${ch.id}" style="font-size:0.65rem; padding:2px 8px; width:100%;">📷 Upload Image</button>` : `<button class="char-upload-btn" data-upload-char="${ch.id}" style="font-size:0.65rem; padding:2px 8px; width:100%;">🔄 Change Image</button>`)}
...
(function initCastSetup() {
  ...
  // ─── Voice catalog + voice helpers (Phase 1 of voice-and-lipsync-plan) ───
  // Hardcoded Gemini voice catalog. ElevenLabs voices populated dynamically
  // when a key is configured (see Phase 3).
  ...
  // Project-level voice config defaults — initialized once per project.
  function _ensureVoiceConfig() {
    if (!window.createJobState) window.createJobState = {};
    ...
    return window.createJobState.voiceConfig;
  }
  window._ensureVoiceConfig = _ensureVoiceConfig;

  // Auto-assign default voice + gender for an entity that doesn't have one.
  function castAssignDefaultVoice(item, gender) {
    if (!item) return;
    const cfg = _ensureVoiceConfig();
    item.gender = gender || item.gender || 'neutral';
    if (!item.voice) {
      const def = cfg.defaultsByGender[item.gender] || cfg.fallbackVoice;
      item.voice = {
  ...
  window.castAssignDefaultVoice = castAssignDefaultVoice;

  // Lookup a voice in the catalog by (provider, voiceId).
  function castFindVoiceInCatalog(provider, voiceId) {
    const list = (window.VOICE_CATALOG && window.VOICE_CATALOG[provider]) || [];
    return list.find(v => v.id === voiceId) || null;
  }
  window.castFindVoiceInCatalog = castFindVoiceInCatalog;

  // Gender inference from name + appearance (Gemini call). Fire-and-forget;
  // returns 'neutral' if anything goes wrong. Called when a character is
  // created without an explicit gender.
  async function castInferGenderFromName(name, appearanceSheet) {
    try {
      const key = (typeof getCreateGeminiKey === 'function') ? getCreateGeminiKey() : null;
      if (!key) return 'neutral';
      if (typeof callGeminiAPI !== 'function') return 'neutral';
      const resp = await callGeminiAPI(['gemini-2.5-flash'], {
        contents: [{ parts: [{ text:
          ...
          `Name: ${name || '(unknown)'}\nDescription: ${(appearanceSheet || '').slice(0, 300)}\n\nAnswer:`
        }] }]
      }, key);
      const ans = (resp && resp.candidates && resp.candidates[0] && resp.candidates[0].content
                   && resp.candidates[0].content.parts && resp.candidates[0].content.parts[0]
                   && resp.candidates[0].content.parts[0].text || '').trim().toLowerCase();
      if (ans.startsWith('male'))   return 'male';
      if (ans.startsWith('female')) return 'female';
      return 'neutral';
    } catch (e) {
      console.warn('[Voice] gender inference failed:', e.message);
      return 'neutral';
    }
  }
  window.castInferGenderFromName = castInferGenderFromName;

  // Sample playback — single audio element pool; only one sample audible
  // at a time across the whole panel. Cached samples in IDB.
  if (!window._castSampleAudio) {
    window._castSampleAudio = null;
    window._castSampleAudioBtn = null;
  }
  async function castPlayVoiceSample(provider, voiceId, btnEl) {
    if (!provider || !voiceId) return;
    // Stop any sample currently playing
    if (window._castSampleAudio) {
      try { window._castSampleAudio.pause(); window._castSampleAudio.currentTime = 0; } catch (_) {}
      if (window._castSampleAudioBtn && window._castSampleAudioBtn !== btnEl) {
        window._castSampleAudioBtn.textContent = '▶';
      }
      // Same button clicked again → toggle off
      if (window._castSampleAudioBtn === btnEl) {
        window._castSampleAudio = null;
        window._castSampleAudioBtn = null;
    ...
    const cacheKey = `voicesample_${provider}_${voiceId}`;
    let url = null;
    if (IDB_AVAILABLE) {
      try { url = await _idbGet(cacheKey); } catch (_) {}
    }
    if (!url) {
      if (btnEl) btnEl.textContent = '⋯';
      try {
        if (provider === 'gemini') {
          ...
          const key = (typeof getCreateGeminiKey === 'function') ? getCreateGeminiKey() : null;
          if (!key) { if (btnEl) btnEl.textContent = '▶'; alert('Gemini API key required for voice sample'); return; }
          if (typeof generateTTSGemini !== 'function') { if (btnEl) btnEl.textContent = '▶'; return; }
          const result = await generateTTSGemini(sentence, voiceId, key);
          // generateTTSGemini returns { base64, mimeType }
          url = `data:${result.mimeType};base64,${result.base64}`;
        } else if (provider === 'elevenlabs') {
          if (typeof window.elevenlabsGenerateSample !== 'function') {
            if (btnEl) btnEl.textContent = '▶';
            alert('ElevenLabs voice playback not yet wired');
            return;
          }
          url = await window.elevenlabsGenerateSample(voiceId);
        } else {
          if (btnEl) btnEl.textContent = '▶';
          return;
        }
        if (IDB_AVAILABLE && url) { try { await _idbPut(cacheKey, url); } catch (_) {} }
      } catch (e) {
        console.warn('[Voice] sample generation failed:', e.message);
        if (btnEl) btnEl.textContent = '▶';
        return;
      }
    }
    if (!url) { if (btnEl) btnEl.textContent = '▶'; return; }
    const audio = new Audio(url);
    window._castSampleAudio = audio;
    window._castSampleAudioBtn = btnEl;
    if (btnEl) btnEl.textContent = '⏸';
    audio.onended = () => {
      if (window._castSampleAudioBtn === btnEl && btnEl) btnEl.textContent = '▶';
      if (window._castSampleAudio === audio) {
        window._castSampleAudio = null;
        window._castSampleAudioBtn = null;
      }
    };
  ...
  window.castPlayVoiceSample = castPlayVoiceSample;
  // ─── End voice helpers ────────────────────────────────────────────────
  ...
  // Voice strip — emitted only for characters / presenter / setting (entities
  // that can speak). Locations don't get a voice. Type 'character' renders.
  function _renderVoiceStrip(item, type) {
    if (type !== 'character') return '';
    castAssignDefaultVoice(item, item.gender);  // ensure defaults present
    const gender = item.gender || 'neutral';
    const voice = item.voice || {};
    const provider = voice.provider || 'gemini';
    const voiceId = voice.voiceId || 'Kore';
    const cfg = _ensureVoiceConfig();
    const elevenlabsAvailable = !!cfg.elevenlabsKeyConfigured && (window.VOICE_CATALOG.elevenlabs || []).length > 0;

    const genderOpts = [
      { v: 'male',    label: '♂ Male' },
      { v: 'female',  label: '♀ Female' },
      { v: 'neutral', label: '⚧ Neutral' },
    ].map(o => `<option value="${o.v}"${gender === o.v ? ' selected' : ''}>${o.label}</option>`).join('');

    // Build voice options: gender-matched first, then "Other voices"
    const list = (window.VOICE_CATALOG[provider] || []);
    const matched = list.filter(v => v.gender === gender);
    const others  = list.filter(v => v.gender !== gender);
    const optStr = (v) => `<option value="${v.id}"${voiceId === v.id ? ' selected' : ''}>${escapeHtml(v.name)} — ${escapeHtml(v.tag)}</option>`;
    const voiceOpts =
      (matched.length ? `<optgroup label="Recommended for ${gender}">${matched.map(optStr).join('')}</optgroup>` : '') +
      (others.length ? `<optgroup label="Other voices">${others.map(optStr).join('')}</optgroup>` : '');

    const providerToggle =
      `<div class="cast-voice-provider">` +
        `<button type="button" class="cast-voice-provider-btn${provider === 'gemini' ? ' active' : ''}" data-action="voice-provider" data-provider="gemini">Gemini</button>` +
        `<button type="button" class="cast-voice-provider-btn${provider === 'elevenlabs' ? ' active' : ''}${elevenlabsAvailable ? '' : ' disabled'}" data-action="voice-provider" data-provider="elevenlabs"${elevenlabsAvailable ? '' : ' title="Configure ElevenLabs API key in voice settings"'}>ElevenLabs${elevenlabsAvailable ? '' : ' 🔒'}</button>` +
      `</div>`;

    return `<div class="cast-voice-strip" data-voice-strip="1">
      ${providerToggle}
      <div class="cast-voice-controls">
        <label class="cast-voice-label">Gender:</label>
        <select class="cast-voice-gender" data-action="voice-gender">${genderOpts}</select>
        <label class="cast-voice-label">Voice:</label>
        <select class="cast-voice-id" data-action="voice-id">${voiceOpts}</select>
        <button type="button" class="cast-voice-play" data-action="voice-play" aria-label="Play voice sample">▶</button>
      </div>
    </div>`;
  }
  ...
  function _renderRowCard(item, type) {
    ...
    return `<div class="cast-row${needsRegen ? ' cast-row-needs-regen' : ''}" data-id="${item.id}" data-type="${type}">
      ...
      <div class="cast-row-actions">
        ${(_isPhotorealisticMode() && isChar) ? `<div style="font-size:0.72rem;color:var(--text-muted);">AI-generated appearance only</div>` : `<button class="btn-xs cast-upload-btn" data-action="upload">${item.uploadedImageDataUrl ? '🔄 Change ref' : '📎 Upload ref'}</button>`}
  ...
  function _wireRowEvents() {
    document.querySelectorAll('#cast-char-rows .cast-row, #cast-loc-rows .cast-row').forEach(rowEl => {
      ...
      // Voice strip — gender + voice + sample play (characters only)
      const genderSel = rowEl.querySelector('select[data-action="voice-gender"]');
      if (genderSel) {
        genderSel.addEventListener('change', () => {
          item.gender = genderSel.value;
          // Auto-update voice to gender-default when user changes gender
          const cfg = _ensureVoiceConfig();
          const def = cfg.defaultsByGender[item.gender] || cfg.fallbackVoice;
          item.voice = Object.assign({}, item.voice, {
            provider: def.provider, voiceId: def.voiceId, voiceName: def.voiceName,
          });
          if (typeof autoSaveCreateState === 'function') autoSaveCreateState();
          renderCastRows();
        });
      }
      const voiceSel = rowEl.querySelector('select[data-action="voice-id"]');
      if (voiceSel) {
        voiceSel.addEventListener('change', () => {
          if (!item.voice) castAssignDefaultVoice(item, item.gender);
          item.voice.voiceId = voiceSel.value;
          const cat = castFindVoiceInCatalog(item.voice.provider, voiceSel.value);
          item.voice.voiceName = cat ? cat.name : voiceSel.value;
          item.voiceLockedAt = new Date().toISOString();
          if (typeof autoSaveCreateState === 'function') autoSaveCreateState();
        });
      }
      rowEl.querySelectorAll('button[data-action="voice-provider"]').forEach(btn => {
        btn.addEventListener('click', (e) => {
          e.preventDefault();
          if (btn.classList.contains('disabled')) return;
          const provider = btn.dataset.provider;
          if (!item.voice) castAssignDefaultVoice(item, item.gender);
          if (item.voice.provider === provider) return;
          item.voice.provider = provider;
          // Pick first matched-gender voice from new provider catalog
          const list = (window.VOICE_CATALOG[provider] || []);
          const matched = list.find(v => v.gender === (item.gender || 'neutral')) || list[0];
          if (matched) {
            item.voice.voiceId = matched.id;
            item.voice.voiceName = matched.name;
          }
          if (typeof autoSaveCreateState === 'function') autoSaveCreateState();
          renderCastRows();
        });
      });
      const playBtn = rowEl.querySelector('button[data-action="voice-play"]');
      if (playBtn) {
        playBtn.addEventListener('click', (e) => {
          e.preventDefault();
          if (!item.voice) return;
          castPlayVoiceSample(item.voice.provider, item.voice.voiceId, playBtn);
        });
      }

      rowEl.querySelectorAll('button[data-action]').forEach(btn => {
        const a = btn.dataset.action;
        // Skip voice-strip actions — already wired above
        if (a === 'voice-gender' || a === 'voice-id' || a === 'voice-provider' || a === 'voice-play') return;
        btn.addEventListener('click', () => {
  ...
  async function _generateOne(item, type) {
    ...
    try {
      ...
      // Phase 1 — voice: infer gender for characters and assign default voice.
      // Locations don't get a voice. Fire-and-forget — non-blocking.
      if (type === 'character' && !item.gender) {
        castInferGenderFromName(item.name, item.appearanceSheet).then(g => {
          if (item.gender) return;  // user may have set it manually meanwhile
  ...
  function _renderBrandSlot(kind, item, slotId, removable) {
    ...
    if (item.locked) {
      ...
      slot.innerHTML = `<div class="cast-row${needsRegen ? ' cast-row-needs-regen' : ''}" data-brand-kind="${kind}">
        ...
        <div class="cast-row-actions">
          ${(kind === 'presenter' && _isPhotorealisticMode()) ? `<div style="font-size:0.72rem;color:var(--text-muted);">AI-generated appearance only</div>` : `<button class="btn-xs" data-brand-action="upload-${kind}">${item.uploadedImageDataUrl ? '🔄 Change' : '📎 Upload ref'}</button>`}
