...
// 9 cell descriptions — exact copy from grid_prompt.py / lora-studio-plan.md §GRID_CELL_DESCRIPTIONS
...
async function _openInputModal(path) {
  ...
  const TRAINER_CFG = {
    ...
    'scene-ai':     { trainerType: 'flux-portrait',  trainerEndpoint: 'fal-ai/flux-lora-portrait-trainer',  inferenceEndpoint: 'fal-ai/flux-lora',  compatibleWith: ['scene'],        trainSteps: 2000 },
  };
...
async function _handleGuidedSlot(itemId, slotIdx, file) {
  ...
  _validateInputModal();
  // Extract appearance when all 4 mandatory slots (0-3) are filled
  const filled = await Promise.all([0,1,2,3].map(i => _idbGet(`lora_v2_${itemId}_photo_${i}`).catch(() => null)));
  if (filled.every(Boolean)) _maybeExtractAppearance(itemId);
}
...
async function _handleOutfitSlot(itemId, oi, si, file) {
  ...
  _validateInputModal();
  // Extract appearance when outfit 0 is fully filled (6 photos)
  if (oi === 0) {
    const slots = await Promise.all(Array.from({length:6}, (_,i) => _idbGet(`lora_v2_${itemId}_photo_${i}`).catch(() => null)));
    if (slots.every(Boolean)) _maybeExtractAppearance(itemId);
  }
}
...
async function _handleAiRef(itemId, refIdx, file, container) {
  ...
  _validateInputModal();
  // Extract appearance from first ref image immediately
  if (refIdx === 0) _maybeExtractAppearance(itemId);
}
...
async function _generatePreviews(itemId) {
  ...
  const identity = _buildIdentityParagraph(item);
  // Suffix: per-feature consistency constraints + finger anatomy + style anchor
  const suffix   = item.type !== 'product' ? _buildPromptSuffix(item) : '';
...
function _renderTuningPanel(itemId) {
  ...
  _renderTuningAppearanceBlock(itemId);
  _renderTuningPreviews(itemId, false);
}
...
// ════════════════════════════════════════════════════
//  APPEARANCE EXTRACTION — Gemini Vision → appearanceBlock
// ════════════════════════════════════════════════════

const APPEARANCE_EXTRACT_PROMPT =
`You are a precise morphological feature extractor for AI image generation.
Analyze ALL attached reference photo(s) of the same person and return ONLY a valid JSON object.
These descriptions are injected verbatim into generation prompts — be specific and exact.

Return this exact shape (no markdown, no extra text):
{
  "age": "estimated age range e.g. '30-35'",
  "gender": "male / female / other",
  "build": "slim / average / athletic / stocky",
  "skinTone": "precise description with undertone e.g. 'warm medium-brown, South Asian complexion'",
  "faceShape": "oval / round / square / heart / oblong",
  "hair": "length, color, texture, style — e.g. 'short, black, neat side-parted, clean-shaven'",
  "eyes": "color and shape — e.g. 'dark brown, almond-shaped'",
  "distinguishingMarks": [
    "each visible mark with PRECISE location — e.g. 'small dark mole on left cheek ~2cm below the eye'",
    "e.g. 'faint horizontal scar on right eyebrow lateral edge'"
  ]
}

Rules:
- distinguishingMarks must list ALL visible moles, scars, birthmarks, tattoos with exact location.
- If no marks are visible return an empty array.
- Never leave a field null — use your best estimate if uncertain.`;

async function _extractAppearanceBlock(itemId) {
  const item = getItemById(itemId);
  ...
  saveItem({ ...getItemById(itemId), appearanceExtracting: true });
  _renderTuningAppearanceBlock(itemId);

  try {
    const parts = [...imgParts, { text: APPEARANCE_EXTRACT_PROMPT }];
    ...
    for (const model of models) {
      try {
        ...
        jsonText = d?.candidates?.[0]?.content?.parts?.[0]?.text || null;
        if (jsonText) break;
      } catch (e) { console.warn(`[AppearanceExtract] ${model} failed:`, e.message); }
    }
    if (!jsonText) throw new Error('No Gemini response');

    const match = jsonText.match(/\{[\s\S]*\}/);
    if (!match) throw new Error('No JSON in response');
    const ab = JSON.parse(match[0]);
    if (typeof ab !== 'object' || !ab.age) throw new Error('Invalid shape');

    saveItem({ ...getItemById(itemId),
      appearanceBlock: { ...ab, extractedAt: Date.now(), extractedFrom: imgParts.length },
      appearanceExtracting: false,
    });
    _showToast('✓ Character profile extracted');
  } catch (e) {
    console.warn('[AppearanceExtract] failed:', e.message);
    saveItem({ ...getItemById(itemId), appearanceExtracting: false });
  }
  _renderTuningAppearanceBlock(itemId);
}

function _renderTuningAppearanceBlock(itemId) {
  const el = document.getElementById('tuning-appearance-block');
  if (!el) return;
  const item = getItemById(itemId);
  if (!item) return;

  if (item.appearanceExtracting) {
    el.innerHTML = `<div class="appearance-extracting">⏳ Extracting character profile…</div>`;
    return;
  }
  const ab = item.appearanceBlock;
  if (!ab) {
    el.innerHTML = `<div class="appearance-empty">Profile auto-extracted after photos are uploaded.</div>`;
    return;
  }
  const marks = (ab.distinguishingMarks || []);
  el.innerHTML = `
    <div class="appearance-profile">
      <div class="appearance-profile-hd">
        <span class="appearance-profile-title">Character Profile</span>
        <span class="lora-optional-badge" style="margin-left:6px">AUTO</span>
        <button class="btn-xs" id="btn-reextract-appearance" style="margin-left:auto">Re-extract</button>
      </div>
      <div class="appearance-fields">
        <div class="appearance-field"><span class="appearance-label">Age / Gender</span><span class="appearance-val">${_hesc((ab.age||'') + ' · ' + (ab.gender||''))}</span></div>
        <div class="appearance-field"><span class="appearance-label">Build</span><span class="appearance-val">${_hesc((ab.build||'') + ' · ' + (ab.skinTone||''))}</span></div>
        <div class="appearance-field"><span class="appearance-label">Face / Eyes</span><span class="appearance-val">${_hesc((ab.faceShape||'') + ' face · ' + (ab.eyes||''))}</span></div>
        <div class="appearance-field"><span class="appearance-label">Hair</span><span class="appearance-val">${_hesc(ab.hair||'')}</span></div>
        ${marks.length ? `<div class="appearance-field"><span class="appearance-label">Marks</span><span class="appearance-val">${_hesc(marks.join(' · '))}</span></div>` : ''}
      </div>
    </div>`;
  el.querySelector('#btn-reextract-appearance')?.addEventListener('click', () => _extractAppearanceBlock(itemId));
}

// Build one-paragraph identity sentence from appearanceBlock (fallback: item.description)
function _buildIdentityParagraph(item) {
  const ab = item?.appearanceBlock;
  if (!ab) return (item?.description || '').trim();
  const parts = [];
  if (ab.age)      parts.push(`${ab.age}-year-old ${ab.gender || 'person'}`);
  if (ab.build)    parts.push(`${ab.build} build`);
  if (ab.skinTone) parts.push(`${ab.skinTone} skin tone`);
  if (ab.faceShape)parts.push(`${ab.faceShape} face shape`);
  if (ab.hair)     parts.push(`hair: ${ab.hair}`);
  if (ab.eyes)     parts.push(`eyes: ${ab.eyes}`);
  const marks = ab.distinguishingMarks || [];
  if (marks.length) parts.push(`distinguishing marks: ${marks.join(', ')}`);
  return parts.join(', ');
}

// Build the consistency enforcement block appended to every character inference prompt.
// Covers: finger anatomy, per-feature maintenance instructions, marks, style anchor.
function _buildPromptSuffix(item) {
  const ab = item?.appearanceBlock;
  const constraints = [];

  // ── Per-feature consistency enforcement ──────────────────────────
  if (ab?.skinTone)  constraints.push(`maintain exact skin tone: ${ab.skinTone}`);
  if (ab?.faceShape) constraints.push(`maintain exact face shape: ${ab.faceShape}`);
  if (ab?.eyes)      constraints.push(`maintain eye color and shape: ${ab.eyes}`);
  if (ab?.hair)      constraints.push(`maintain hair exactly: ${ab.hair}`);
  const marks = ab?.distinguishingMarks || [];
  if (marks.length) {
    constraints.push(
      `all facial marks must be present and consistent in exact locations: ${marks.join('; ')}`
    );
  }

  // ── Finger / hand anatomy ─────────────────────────────────────────
  constraints.push('exactly 5 fingers on each hand — no extra, no missing, no fused fingers');

  // ── Style anchor (DOE character_spec.py style_anchor) ────────────
  constraints.push(
    'photorealistic, sharp focus, natural skin texture with visible pores, ' +
    'no airbrushing, no plastic skin, no exaggerated facial features, ' +
    'no stylized rendering, no anime, no caricature'
  );
...
// ── Trigger extraction after mandatory photos are filled ──────────
// Called from upload handlers below when threshold is met.
function _maybeExtractAppearance(itemId) {
  // Debounce: don't re-extract if block is less than 5 minutes old
  const item = getItemById(itemId);
  if (!item) return;
  if (item.appearanceExtracting) return;
  const age = item.appearanceBlock?.extractedAt
    ? Date.now() - item.appearanceBlock.extractedAt : Infinity;
  if (age < 5 * 60 * 1000) return;
  _extractAppearanceBlock(itemId);
}
...
function _buildGridPromptsForItem(item, outfitTheme) {
  ...
  return GRID_CELL_POSES.map(pose => `${prefix}${outfitTheme.desc}, ${pose}`);
}
...
The ${refParts.length} reference image(s) attached above show this EXACT character.
