    ...
    try {
      ...
      for (const s of segments) {
        if (s.words && s.words.length > 0) {
          ...
          const sDur = (s.endTime || 0) - (s.startTime || 0);
          const wDur = sDur / wds.length;
          wds.forEach((w, wi) => { words.push({ word: w, start: s.startTime + wi * wDur, end: s.startTime + (wi + 1) * wDur }); });
        }
      }
...
function renderReelFrame(time) {
  ...
  // Use a small offset to ensure time=0 matches first word/scene
  ...
  if (scene) {
    if (scene.isVideo && reelVideoEl) {
      // Video mode: seek to actual video position
      const vidOffset = activeResult ? activeResult.videoStart : 0;
      const seekTime = vidOffset + t;
      // Seek and draw after seek completes
      if (Math.abs(reelVideoEl.currentTime - seekTime) > 0.05) {
      ...
      return; // async path — subtitles drawn after seek
  ...
  // Subtitles (sync path for non-video)
  drawReelSubtitles(ctx, platform, t);
}
