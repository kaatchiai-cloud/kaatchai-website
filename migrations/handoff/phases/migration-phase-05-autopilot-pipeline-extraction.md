# Phase 05 — Async Job Infrastructure + AutoPilot Migration

> **Rev-5 changes (2026-05-16):** Added `js/21-kling.js:356` Gemini continuation-prompt call to P05 scope. Added ElevenLabs STT (`/v1/audio/transcribe`) and voice catalog (`/v1/audio/voices`) endpoints. Added explicit Lyria 3 BGM exit-criteria verification. Added fal.ai carve-out per M4 Option B (deferred to P06). Updated `17c` line count to 5,581; four `17*` files total ~14,000 lines. Added exit-criteria grep for `21-kling.js` Gemini calls, `17a` ElevenLabs calls, and Lyria round-trip.
...
> **Goal in one line:** build the server-side async job infrastructure (submit → poll → done, idempotency, Cloud Run workers, R2 storage, provider routing) and migrate AutoPilot as the first and heaviest consumer to prove the pattern end-to-end. All subsequent pipeline migrations in P06 reuse this infrastructure.
> **Source:** `/Users/praveen/Desktop/stori/migrations/migration-plan.md` Part 2 row 04; coverage matrix pipeline-extraction inventory row "AutoPilot Reel pipeline"; [OVERRIDES] O3, O10, O16.
...
2. **`/v1/jobs/scene-images`** — long-running job that generates scene images for every active storyboard in the project. Server-side calls to Gemini imageGen (per migration-original-spec.md L136–144 grid-gen pattern, but updated for Cloud Run instead of Vercel since grid gen can take 50+ seconds and we want headroom for retries).
...
5. **AutoPilot Reel client cutover** — `js/20-reels-creator.js` (5,847 lines) and the four `js/17a-d-create-*.js` files (~1,981 + ~5,126 + 5,581 + ~1,312 ≈ 14,000 lines, rev-5) replace every direct `fetch('https://generativelanguage.googleapis.com/...')` and direct Kling call with `callApi(...)`. **Line counts drift as the codebase evolves** — re-verify with `wc -l` at phase kickoff. Verified counts (fresh grep this phase): see §6.
...
11. **`js/21-kling.js:356` — Gemini continuation-prompt call (rev-5).** `js/21-kling.js` contains a direct `fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=...')` at line 356, used to generate a continuation prompt for the next video clip by analysing the last frame. **This Gemini call must be replaced with `callApi('/v1/gemini/generate-content')` in P05.** Note: the Kling video submission endpoints (`KLING_BASE/videos/image2video`) already go through the Vercel proxy (`/api/kling`) — only the Gemini call at line 356 needs migration. The server-side JWT migration (item 8b) and this Gemini replacement are the two P05 touch-points in `21-kling.js`.
12. **`/v1/audio/transcribe` — ElevenLabs STT endpoint (rev-5).** `js/17a-create-api.js:468` calls `api.elevenlabs.io/v1/speech-to-text` (Scribe v1) for the Create Story transcription step. This is a **sync** endpoint — transcription returns inline. P05 replaces this direct call with `callApi('/v1/audio/transcribe')`. The Cloud Run handler proxies to ElevenLabs Scribe v1 server-side. A Gemini fallback (at `17a:493`) remains client-side until P06/P07 (it's a `callGeminiAPI` call site already covered by item 8c).
13. **`/v1/audio/voices` — ElevenLabs voice catalog endpoint (rev-5).** `js/17a-create-api.js:753` calls `api.elevenlabs.io/v1/voices` to populate `VOICE_CATALOG.elevenlabs`. P05 adds a server-side endpoint that fetches the catalog (with server-side cache — the catalog is static and small) and returns the JSON. Client replaces the direct fetch with `callApi('/v1/audio/voices')`. The `stori_cast_images_v1` IDB caching (`elevenlabs_voice_catalog` key) is updated to cache the server response instead.
14. **Lyria 3 BGM — explicit coverage (rev-5).** Lyria 3 BGM generation is called from `js/17c-create-pipeline.js:4238` and `js/20-reels-creator.js:4604`. Both call `generativelanguage.googleapis.com/v1beta/models/lyria-3-clip-preview:generateContent`. These are covered by the existing `/v1/jobs/bgm` endpoint (item 7) — the Cloud Run `lyria.js` provider handles the Lyria model call. **Exit criterion added:** Lyria 3 BGM round-trip works end-to-end for both the Create Story path (`17c`) and the Reel BGM path (`20`).
15. **P05 fal.ai carve-out — M4 Option B (rev-5).** fal.ai LoRA inference calls in `js/17c-create-pipeline.js:2648-2750` (`generateLoraImage()`) are **deferred to P06** (LoRA Studio pipeline). P05 exit criteria verify "zero direct AI fetches" for **Gemini and ElevenLabs only** in `17c`. fal.ai calls in `17c` are gated by `window.LoraLibrary` readiness check and only fire when a trained LoRA is assigned — they are safe to defer. After P05, `17c` will ship with a mix of `callApi()` (Gemini + ElevenLabs) and direct `fetch` (fal.ai LoRA) until P06 completes.
...
| 4 | Zero direct `fetch` to `generativelanguage.googleapis.com` in `js/17a-create-api.js`, `js/17b-create-references.js`, `js/17c-create-pipeline.js`, `js/17d-create-languages.js`, `js/20-reels-creator.js`, **`js/21-kling.js`** (rev-5). (Direct Kling calls in `js/21-kling.js` also gone.) Verified by `grep`. | Fresh grep. |
| 4a | **(rev-5)** `grep -n "generativelanguage.googleapis.com" js/21-kling.js` returns **0 hits** — Gemini continuation-prompt call at `:356` replaced with `callApi()`. | `grep`. |
| 4b | **(rev-5)** `grep -n "api.elevenlabs.io" js/17a-create-api.js` returns **0 hits** — covers STT (`:468`), voices (`:753`), TTS, voice clone. | `grep`. |
| 4c | **(rev-5)** Lyria 3 BGM round-trip works end-to-end for both Create Story (`17c:4238`) and Reel BGM (`20:4604`) paths via `/v1/jobs/bgm`. | Manual + recorded run. |
...
│  /v1/jobs/bgm             ──┤                                    │
│  /v1/audio/transcribe     ──┤── sync (ElevenLabs STT)            │
│  /v1/audio/voices         ──┤── sync (ElevenLabs catalog)        │
│  GET /v1/jobs/:id         ──┘                                    │
│                                                                  │
...
└────────┬───────────────────────────────────────────────┬─────────┘
   ...
   └──────────┘                           │  - Lyria (BGM)            │
                                          │     - ElevenLabs (STT, voices, TTS later) │
                                          └──────────┬────────────────┘
                                                     │
                                                     ▼
...
#### B.5 `infra/cloud-run/providers/elevenlabs.js` (rev-5)
- [ ] `elevenlabsTranscribe({audioUrl, model='scribe_v1'})` → returns transcript text + word-level timestamps. Sync call to `api.elevenlabs.io/v1/speech-to-text`.
- [ ] `elevenlabsVoices()` → returns voice catalog JSON. Server-side cached (TTL ≈ 24h; catalog is static and small).
- [ ] Reads `ELEVENLABS_API_KEY` from Cloud Run env.
...
#### C.4a `POST /v1/audio/transcribe` (rev-5)
- [ ] Sync endpoint. Accepts audio payload, proxies to ElevenLabs Scribe v1 (`api.elevenlabs.io/v1/speech-to-text`).
- [ ] Returns transcript text + word-level timestamps inline.
- [ ] Reads `ELEVENLABS_API_KEY` from Cloud Run env.

#### C.4b `GET /v1/audio/voices` (rev-5)
- [ ] Sync endpoint. Server-side fetches ElevenLabs voice catalog, caches response (TTL ≈ 24h).
- [ ] Returns catalog JSON. Client populates `VOICE_CATALOG.elevenlabs` from this response.
...
- [ ] **`js/21-kling.js:356` Gemini continuation-prompt call (rev-5):** replace the direct `fetch('https://generativelanguage.googleapis.com/.../gemini-2.5-flash:generateContent')` at line 356 with `callApi('/v1/gemini/generate-content')`. Kling video submission endpoints (`KLING_BASE/videos/image2video`) already go through the Vercel proxy (`/api/kling`) — no change needed there.
- [ ] **`js/17a-create-api.js:468` ElevenLabs STT (rev-5):** replace direct `fetch('https://api.elevenlabs.io/v1/speech-to-text')` with `callApi('/v1/audio/transcribe')`. The Gemini fallback at `:493` is a `callGeminiAPI` site already covered by the `callGeminiAPI` sweep above.
- [ ] **`js/17a-create-api.js:753` ElevenLabs voice catalog (rev-5):** replace direct `fetch('https://api.elevenlabs.io/v1/voices')` with `callApi('/v1/audio/voices')`. Update `VOICE_CATALOG.elevenlabs` initialization to populate from server response. The `stori_cast_images_v1` IDB caching (`elevenlabs_voice_catalog` key) caches the server response instead.
- [ ] **Lyria 3 BGM (rev-5):** both `js/17c-create-pipeline.js:4238` and `js/20-reels-creator.js:4604` call `generativelanguage.googleapis.com/.../lyria-3-clip-preview:generateContent` — these are replaced by `callApi('/v1/jobs/bgm')` and covered by the existing `/v1/jobs/bgm` endpoint + `lyria.js` provider.
- [ ] **fal.ai carve-out (rev-5, M4 Option B):** `js/17c-create-pipeline.js:2648-2750` (`generateLoraImage()`) calls fal.ai directly. These are **deferred to P06** (LoRA Studio pipeline). P05 does NOT replace them. They are gated by `window.LoraLibrary` readiness and only fire when a trained LoRA is assigned — safe to defer.
...
3. `grep -rn "generativelanguage.googleapis.com" js/17a-create-api.js js/17b-create-references.js js/17c-create-pipeline.js js/17d-create-languages.js js/20-reels-creator.js js/21-kling.js` returns 0 hits at end of phase. (Other files like `28-canvas-consistency.js`, `26-brainstorm.js`, `30-lipsync.js`, `32-audio-input.js`, `33-audio-rehearsal.js`, `34-lora-library.js` may still have direct calls — those are Phase 06's responsibility. Per fresh grep at start of phase: total `generativelanguage.googleapis.com` refs = 21+ across files — the AutoPilot files account for some subset, to be itemized at kickoff.)
4. `grep -n "api.elevenlabs.io" js/17a-create-api.js` returns 0 hits — STT, voices, TTS, and voice clone all routed through `/v1/*`. (rev-5)
5. Canary drill executed; runbook checked in.
6. 4-scene Illustrated reel round-trips and exports MP4.
7. 4-scene Animated reel (flag on) round-trips, including BGM, and exports MP4.
8. **(rev-5)** Lyria 3 BGM round-trip works end-to-end for both Create Story path (`17c:4238`) and Reel BGM path (`20:4604`).
...
### Predecessors
- **Phase 04 (Module Split)** is the immediate predecessor. P04 already lifted `openCanvasPanel` / `closeCanvasPanel` / `_callGeminiForVideoPrompts` / `cgFillVideoPrompts` / `cgLaunchVideoAgent` out of `js/17c-create-pipeline.js` into `js/17e-canvas-launch.js`, and the Tier-2 fal.ai lipsync block (current `3728–3900` in `17c`) into `js/17f-tier2-lipsync-fal.js`. **This phase does NOT touch `17e` or `17f`** — those belong to the editor (still client-side) and to P06 (server replacement) respectively. Without P04 having run, this phase would collide with both.
...
- New: `infra/supabase/migrations/0003_jobs.sql`, `infra/cloud-run/routes/jobs.js`, `infra/cloud-run/worker/loop.js`, `infra/cloud-run/providers/{gemini,kling,veo3,lyria,elevenlabs}.js`, `infra/cloud-run/runbooks/canary.md`, ADR-02, ADR-04 (drafted in P05, finalized in P07).
- Modified: `js/17a-create-api.js`, `js/17b-create-references.js`, `js/17c-create-pipeline.js` (5,581 lines current — rev-5; after P04 split estimate: 5,581 − 158 − 173 = 5,250), `js/17d-create-languages.js`, **`js/17e-canvas-launch.js`** (rev-4: option (a) — Animated AutoPilot path lives in P05; replace `_callGeminiForVideoPrompts` + `cgFillVideoPrompts` + `cgLaunchVideoAgent` direct Gemini calls with `callApi('/v1/...')`), `js/20-reels-creator.js`, **`js/21-kling.js`** (rev-4: delete `generateKlingJWT()` + the two `stori_kling_*` localStorage reads at `:13–14`; replace the two `await generateKlingJWT()` call sites at `:37, :70` with `callApi('/v1/jobs/animation' | /v1/jobs/<id>)`; server-side Kling auth lives in Cloud Run env. **rev-5:** also replace Gemini continuation-prompt call at `:356` with `callApi('/v1/gemini/generate-content')`). (Hands off the BYOK helper functions for Gemini; only `fetch` + `callGeminiAPI` call sites change in the four `17*` + `20` + `21` files.)
...
- **fal.ai carve-out (rev-5, M4 Option B):** `js/17c-create-pipeline.js:2648-2750` (`generateLoraImage()`) calls fal.ai directly — these are **deferred to P06** (LoRA Studio pipeline). P05 exit criteria verify "zero direct AI fetches" for Gemini and ElevenLabs only in `17c`. fal.ai calls in `17c` are gated by `window.LoraLibrary` readiness check and only fire when a trained LoRA is assigned — they are safe to defer.
...
4. **Veo3 vs Kling default** — Kling is mentioned in migration-original-spec.md L20; Veo3 is mentioned in redesign-plan.md L92. Both are listed as options. [non-blocking — start with Kling-only; add Veo3 in Phase 06 or a follow-up if dogfood demands it].
...
- Successor: `/Users/praveen/Desktop/stori/migrations/migration-phase-06-secondary-pipelines-extraction.md`
...
- Source spec: `/Users/praveen/Desktop/stori/migrations/migration-original-spec.md` §New Files §Backend L132–158, §Modified Files L191–207, §Notes L351–356; coverage matrix pipeline-extraction inventory row 1.
- ADRs: `migration-adr-02-long-running-jobs.md`, `migration-adr-04-trunk-based-canary.md` (drafted here, finalized in P07)
- Audit: `migration-plan-audit-rev5-merged.md` (rev-5 source for H1, M4, S6, S12, S13 findings applied here)
