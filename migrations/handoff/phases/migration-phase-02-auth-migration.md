...
> **Source:** `/Users/praveen/Desktop/stori/migrations/migration-plan.md` Part 2 row 02; coverage matrix rows 6, 7, 11; [OVERRIDES] O14.
...
4. **Fake-auth-stub removal at the verified location.** **The line range drifts as the file grows** — the audit trail saw the stub at `1174-1177` (rev-1, wrong), then `~1417–1472 (behavioural verification: `grep stori_user` returns 0)` (rev-3, correct at the time), now **`~1417–1472`** (rev-4 pass-2 verification 2026-05-06: `localStorage.setItem('stori_user', ...)` at 1431; `localStorage.removeItem('stori_user')` at 1435; `localStorage.getItem('stori_user')` at 1444; `updateUserSection()` definition + invocation through 1472). **Use the behavioural exit criterion** rather than line numbers: `grep -n "stori_user\|btnSignIn\|btnSignOut\|btnManageSub\|updateUserSection" js/15-project.js` returns **0 hits** after the deletion. The actual block to delete includes: the `btn-sign-in` DOM binding (around line 1417), the `btnSignIn`/`btnSignOut`/`btnManageSub` event handlers, the `updateUserSection()` function definition, and its trailing invocation. Phase 02 replaces this block with the `js/00-auth.js`-driven flow.
...
| 4 | Lines ~1417–1472 (behavioural verification: `grep stori_user` returns 0) of `js/15-project.js` (the fake-auth block plus `updateUserSection`) are deleted; equivalent behaviour lives in `js/00-auth.js`. `grep -n stori_user js/15-project.js` returns zero matches. | `grep` + diff review. |
...
- The `localStorage.getItem('stori_user')` pattern (currently at `js/15-project.js:1444`; line drifts as the file grows — re-verify by `grep -n "localStorage.getItem('stori_user')" js/15-project.js` at kickoff) is replaced by Supabase session storage (Supabase JS SDK manages localStorage under its own key — no manual juggling).
...
|---------|--------|-----------|--------------|
| Auth provider | **Supabase Auth** | Spec mandates (migration-original-spec L13). | Auth0, Clerk: extra vendor, extra cost. |
...
- [ ] Create the file. Add `<script src="https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2"></script>` to `index.html` head (per migration-original-spec L212).
...
- [ ] Delete the block ~1417–1472 (behavioural verification: `grep stori_user` returns 0) (the `btnSignIn` / `btnSignOut` handlers that fake-write to `localStorage`, plus the `updateUserSection()` definition + its trailing invocation on line 1404).
...
- [ ] Add the Supabase CDN script tag (per migration-original-spec L212).
...
- Modified:
  - `js/15-project.js` — delete lines ~1417–1472 (behavioural verification: `grep stori_user` returns 0) (auth stub + `updateUserSection`).
...
- Predecessor: `/Users/praveen/Desktop/stori/migrations/migration-phase-01-backend-foundations.md`
- Successor: `/Users/praveen/Desktop/stori/migrations/migration-phase-03-api-contract-and-project-state.md`
- Source spec: `/Users/praveen/Desktop/stori/migrations/migration-original-spec.md` §New Files Frontend L103–116, §Modified Files §`js/15-project.js` L179–183 (line range corrected — see Part 4 of phase index), §Security Non-Negotiables L248–292 (rows 1, 2, 5, 6 only — billing rows 3 + 4 stripped per O15)
- Verified line range: `js/15-project.js:~1417–1472 (behavioural verification: `grep stori_user` returns 0)` (auth stub + `updateUserSection`); spec's `1174-1177` is wrong (lines 1174–1177 are PiP video restoration). See `/Users/praveen/Desktop/stori/migrations/devDoc-migration/.architect/spec-inventory.md` Part 5 unanchored claim #1.
- ADRs: `/Users/praveen/Desktop/stori/migrations/migration-adr-05-auth-and-session.md`, `/Users/praveen/Desktop/stori/migrations/migration-adr-03-api-contract.md`
