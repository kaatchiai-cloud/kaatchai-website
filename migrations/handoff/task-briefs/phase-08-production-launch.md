...
### CP-08-1: Monitoring + runbooks (Wk 1–2)
Sentry dashboards live; runbooks for 4 failure modes written; operational status page.
```
grep -c "auth outage\|Cloud Run job stuck\|R2 region failure\|AI provider rate-limit" infra/runbooks/oncall.md
# → ≥ 4
```
HALT: if any runbook scenario missing → cannot ship without all 4 covered.

### CP-08-2: Canary drill on production (Wk 2–3)
5/50/100 release drill on real production Cloud Run; at least one revision promoted.
```
grep -c "5%\|50%\|100%" infra/cloud-run/runbooks/canary.md
# → ≥ 3 entries
```
Smoke: deploy → split 5% traffic → verify error rate < 0.5% → scale to 50% → verify → scale to 100%.
HALT: if error rate > 0.5% at any stage → stop promotion, investigate, rollback.

### CP-08-3: Mock incident + post-mortem (Wk 3–5)
Dry-run incident executed; post-mortem template filed.
```
ls infra/runbooks/dry-runs/
# → at least one mock-incident file
```
Smoke: simulate Cloud Run job stuck → follow runbook → verify resolution; write post-mortem.
HALT: if mock incident reveals runbook gap → update runbook before shipping.
