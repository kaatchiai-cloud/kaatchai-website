...
### CP-01-1: Infra pillars up (Wk 1–2)
Supabase project + `users` table + Cloud Run `/v1/health` + R2 bucket created + Sentry both projects.
```
...
# → true
```
HALT: if any pillar fails to create (e.g., Cloud Run deploy fails 3×) → escalate for account/region issue.

### CP-01-2: Round-trips proven (Wk 2–3)
R2 presigned PUT/GET smoke passes; Sentry events flow from Cloud Run + browser; cold-start < 5s.
```
node scripts/smoke-r2.mjs
# → exits 0; prints "PUT ok ... GET ok ... bytes match"
curl -X POST https://$CLOUDRUN_URL/v1/sentry-smoke
# → 500; event in Sentry stori-cloudrun within 60s
```
HALT: if R2 presign fails → check API token scope; if Sentry events don't land → check DSN env vars.

### CP-01-3: Repo bootstrap + CI (Wk 3–5)
pnpm workspaces, TS refs, vitest, playwright, eslint, prettier, CI green, branch protection.
```
pnpm install --frozen-lockfile && pnpm typecheck && pnpm lint && pnpm test && pnpm test:e2e
# → all green
git push --no-verify origin main 2>&1 | grep -q "rejected"
# → true (branch protection works)
```
HALT: if CI red on sample PR → fix before declaring phase done.
