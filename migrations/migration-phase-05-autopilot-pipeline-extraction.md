...
5. **AutoPilot Reel client cutover** — `js/20-reels-creator.js` (5,847 lines) and the four `js/17a-d-create-*.js` files (1,830 + 5,114 + 5,206 + 1,312 = 13,462 lines, rev-4 pass-2) replace every direct `fetch('https://generativelanguage.googleapis.com/...')` and direct Kling call with `callApi(...)`. **Line counts drift as the codebase evolves** — re-verify with `wc -l` at phase kickoff. Verified counts (fresh grep this phase): see §6.
