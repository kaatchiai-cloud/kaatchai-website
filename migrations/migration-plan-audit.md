...
|----------|---------------------|
| migration-original-spec.md L110, L243 | 1174-1177 |
...
The migration-original-spec.md still says 1174-1177 (the old, wrong range). The plan corrects it to 1363-1404 but misses line 1362 (`if (btnSignIn)`). The inventory says "1363, 1367, 1374, 1404" which are just the `stori_user` lines, not the full block.

**Fix:** Update migration-original-spec.md line references. Use "lines 1362-1404" consistently everywhere.

---

### 9. `migration-original-spec.md` Is Significantly Stale and Conflicts With the Plan

The migration-original-spec.md (358 lines) was the original spec but now conflicts with the plan in multiple ways:
...
**Fix:** Add a prominent header banner to migration-original-spec.md stating it is superseded by the migration plan and ADR documents. The plan's out-of-scope table (Part 5) marks these as "deferred" or "superseded" but doesn't warn a reader that the document's architecture section is actively misleading.
...
The existing `api/kling.js` (lines 7-9) sets `Access-Control-Allow-Origin: *`. This is acknowledged as a security issue in migration-original-spec.md ("CORS locked to `https://stori.app` — NOT `*`"). However:
...
8. **Update migration-original-spec.md** with a prominent banner stating it's superseded. Fix the stale line references (1174-1177 → 1362-1404), stale call-site counts, and conflicting architecture description.
...
- The migration details spec (`migration-original-spec.md`)
