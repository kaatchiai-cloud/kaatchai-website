...
6. **`reel_projects`** — saved reel-builder projects.
   ...
   - **Source:** `js/20-reels-creator.js:4363–4481` (save handler), `js/20-reels-creator.js:5477–5515` (load handler).
...
8. **`reference_library`** — per-user cross-project reference library (currently localStorage `stori_ref_library_v1`).
   ...
   - **Source:** `js/17b-create-references.js:4730–4785` (`LIB_KEY = 'stori_ref_library_v1'`, `LIB_CAP = 30`).
...
**Reel→Editor handoff (now an API call, not a table):** the IndexedDB `stori_db` blob handoff at `js/20-reels-creator.js:5796–5839` plus `index.html:4782–4800` is replaced by `POST /v1/projects/import-reel/:reel_project_id` (or a session-state token — final shape decided in ADR-03). No new table; the endpoint materializes a `projects` row from a `reel_projects` row atomically. **Spike-deferred question (revision 3):** Option A (server-materialize) vs Option B (handoff token). **Closed: default A** unless ADR-03 surfaces a reason otherwise.

**RLS on the 5 new tables** is `auth.uid() = user_id` (or transitively via `project_id` for `audio_inputs` / `audio_rehearsals`). `reference_library` is **strictly per-user** — no team / project scoping.

**Cap enforcement** on `reference_library` is via a `BEFORE INSERT` trigger that deletes the oldest row when count would exceed 30, mirroring the client `_libRead/_libWrite` cap eviction at `js/17b-create-references.js:4730–4785`.
...
- Source spec: `/Users/praveen/Desktop/stori/migration-details.md` §Schema L47–97 (billing rows excluded per O15)
