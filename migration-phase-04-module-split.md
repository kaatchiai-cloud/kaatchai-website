...
> **Source:** `/Users/praveen/Desktop/stori/migration-plan.md` Part 2 row 04; revision-3 audit (`migration-plan-audit-report.md`); not in `migration-details.md` — this phase exists to de-risk the heavy P05 refactor.
...
- `js/17c-create-pipeline.js` — **5,199 lines.** P05 extracts the AutoPilot pipeline core; P06 extracts Tier-2 lipsync (currently at lines 3721–3900 of this same file). Without the split, both phases edit the same file in conflicting ways.
...
1. **Split `js/17c-create-pipeline.js` (5,199 lines)** into three files:
   - `js/17c-create-pipeline.js` — AutoPilot pipeline core only (~3,200 lines remain after extraction).
   - **NEW** `js/17e-canvas-launch.js` — owns the canvas-launch surface:
     - `openCanvasPanel` (currently at `js/17c-create-pipeline.js:4891`)
     - `closeCanvasPanel` (currently at `js/17c-create-pipeline.js:4925`)
     - `_callGeminiForVideoPrompts` (currently at `js/17c-create-pipeline.js:4936`)
     - `cgFillVideoPrompts` (currently at `js/17c-create-pipeline.js:4976`)
     - `cgLaunchVideoAgent` (currently at `js/17c-create-pipeline.js:4997`)
     - The `window.openCanvasPanel = ...` / `window.closeCanvasPanel = ...` exports at the bottom of the canvas block.
     - **Total ~145 lines moved** (verified range `4889–5079` in current file; the original audit cited `4889–5029` as a conservative lower bound — verify exact range at kickoff via grep).
   - **NEW** `js/17f-tier2-lipsync-fal.js` — owns Tier-2 lipsync (the fal.ai-hosted Kling LipSync path):
     - All code from the section header `// ─── Lip sync — Phase 8: Tier 2 (Kling LipSync via fal.ai) ─────────────` (line 3721) through its terminating section marker.
     - Includes the fal.ai submit, poll, result fetch helpers + per-scene Tier-2 sync + Tier-2 batch orchestrator.
     - **Total ~180 lines moved** (verified range `3721–3900` in current file). This whole module is destined for replacement by `/v1/jobs/lipsync` in P06; isolating it here means P06 can delete one file rather than carve out lines from a 5,199-line file.
...
|---|----------------|--------------|
| 1 | `js/17c-create-pipeline.js` is reduced to AutoPilot pipeline core (~3,200 lines remain). | `wc -l js/17c-create-pipeline.js` returns ~3,200 ± 50. |
...
```
   ...
   │  ├─ Tier-2 lipsync (fal.ai)              (lines 3721–3900)│  ← P06 territory
   ...
   └──────────────────────────────────────────────────────────┘
                    ...
                    After (post-split)
   ┌────────────────────────────────────┐
   │ js/17c-create-pipeline.js (~3,200) │  AutoPilot core only — P05 ports this
   └────────────────────────────────────┘
   ┌────────────────────────────────────┐
   │ js/17e-canvas-launch.js  (~145)    │  canvas-panel UI launcher — stays client
   └────────────────────────────────────┘
   ┌────────────────────────────────────┐
...
- [ ] Run `grep -n "// ─── Lip sync — Phase 8" js/17c-create-pipeline.js` to find Tier-2 start (verified at line 3721 on 2026-05-06).
...
- [ ] Copy the Tier-2 lipsync block (lines `3721–3900` plus any helpers that ONLY this block uses) verbatim.
...
- [ ] Final line count should be approximately `5199 - 145 - 180 = 4874`. The audit's "~3,200 lines remain" target assumes additional related code moves with the canvas block; if the verified-only-the-listed-functions move is closer to 4,874, that's the true post-split size — update §2 exit criterion 1 to match. **Do not move more code than the spec lists** to chase the 3,200 target. (Note: the 3,200 figure likely included unlisted neighbouring code; budget the actual reduction empirically.)
...
- [ ] Open `index.html`. Locate the **dynamic loader** at lines 4731–4744 (the `var scripts = [...]` array inside `loadEditorScripts`).
...
- [ ] **For `js/28a-image-gen-shim.js`**: locate the **eager** `<script>` block where `js/28-canvas-consistency.js` sits at line 4718 (currently eager).
...
- [ ] **Engineer:** record post-split line counts of all five affected files in `phase-04-post-split-counts.md` (working file). Confirm `wc -l` totals for `(17c + 17e + 17f) ≈ 5199 ± 50` and `(28 + 28a) ≈ 224 ± 30` — i.e., near-zero net delta.
...
- Modified: `js/17c-create-pipeline.js`, `js/28-canvas-consistency.js`, **`index.html` (loader array at 4731–4744 + eager `<script>` block at ~4717–4718)**. **`build.js` is NOT modified** — there is no `MAIN_FILES` symbol; build.js auto-discovers via its existing scans.
...
- `migration-details.md`, `migration-plan.md`, `app/redesign-plan.md`, `app/*-mobile-mockup*.html`.
...
2. **Should we also split `js/20-reels-creator.js` (5,839 lines)?** [non-blocking — NO. P05 ports this whole file server-side; splitting now would be wasted churn.]
...
- Audit driving this phase: `/Users/praveen/Desktop/stori/migration-plan-audit-report.md`
- Predecessor: `/Users/praveen/Desktop/stori/migration-phase-03-api-contract-and-project-state.md`
- Successor: `/Users/praveen/Desktop/stori/migration-phase-05-autopilot-pipeline-extraction.md`
- Source code (verified 2026-05-06):
  - `/Users/praveen/Desktop/stori/js/17c-create-pipeline.js` (5,199 lines; canvas-launch at 4889–5079, Tier-2 lipsync at 3721–3900)
  ...
  - `/Users/praveen/Desktop/stori/index.html` (loader array at 4731–4744; eager `<script>` block at ~4717–4718 — this is where the three new files get wired in)
